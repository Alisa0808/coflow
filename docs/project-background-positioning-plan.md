...
- 当前实现先生成 `.coflow/frame-inputs/frame-request-*.json`；
...
> CoFlow 是一个本地优先的 AI 创作画布插件。它把图片生成、标注修图、视频帧反馈、3D 预览和多模型生成放到同一条 agent 工作流里：你在画布上指，Codex 去理解和生成，结果自动回到画布。
...
视频模型本身可能需要分钟级生成时间，这属于 provider/model latency，不能被画布工程完全消除。但用户不应该为 Codex/白板/上传下载/writeback 的工程链路额外等待分钟级。

因此本项目把生成链路拆成两段可优化 fast path：

```text
用户请求
→ 读取 frame / selection / viewport context
→ 解析素材引用与缓存
→ 构造 provider payload
→ 提交 Atlas / provider job
```

目标：

- 已缓存素材：P50 < 2 秒，P95 < 5 秒；
- 未缓存但为普通图片：P95 < 10 秒；
- 首次大视频 / 4K 大图上传：允许超过 10 秒，但必须进入异步上传/缓存状态，不阻塞用户继续操作。

模型完成后的回板链路：

```text
provider completed
→ 获取 remote output URL
→ 立即插入 native tldraw image / video asset
→ 写入 job / prompt / model / timing metadata
→ 后台 materialize 到本地 asset store
```

目标：

- provider 返回结果后，画布可见 writeback P50 < 2 秒，P95 < 5 秒；
- 不应为了等待完整视频下载而阻塞用户看到结果；
- 本地持久化可以后台完成，但 metadata 必须记录 remote URL、本地路径、状态与耗时。

阶段性实现原则：

- 不重复上传同一个本地素材；能复用 remote asset URL 时必须复用；
- 不把 frame 截图作为唯一输入，结构化 context 和原始素材路径/URL 才是 source of truth；
- 视频生成一律按异步 job card / progress 状态处理，不让 Codex turn 长时间“空等”；
- provider router 收口时统一 Atlas image / video polling 默认间隔为 `2000` ms；这只是完成检测频率，不代表模型生成耗时；
- 每次真实 provider 调用必须记录 `contextReadDurationMs`、`payloadBuildDurationMs`、`uploadDurationMs`、`submitDurationMs`、`pollDurationMs`、`downloadDurationMs`、`writebackDurationMs`，以便判断慢在哪里。
...
- `coflow-image`：同时处理文生图、图生图、参考图生成、图片编辑，由 skill 根据 Frame Input / selection / 用户提示判断模式；
- `coflow-video`：同时处理文生视频、图生视频、参考生视频、视频再生成 / keyframe-guided revision，由 skill 根据输入素材类型和 provider 能力判断模式；
- `coflow-open`：只负责打开白板。
...
- 高级用户可以接入 Codex / GPT Image、Google、Seedance、Kling、Replicate、ComfyUI、Runway、OpenAI、Higgsfield 或内部模型；
- Atlas 示例和文档最完整，默认推荐。

### 6.2 Provider Router 与安装后 onboarding

生成 skill、场景 skill 和底层 provider 不应该强绑定。

用户可能调用的是：

- `coflow-image` / `coflow-video` 这类基础生成 skill；
- Higgsfield marketing skill、产品摄影 skill、广告素材 skill 等场景 skill；
- 用户自己写的内部工作流 skill；
- 未来某个第三方 MCP / CLI / API skill。

但这些 skill 不应该各自内置一套不可替换的模型供应商。更好的结构是：

```text
Scene / generation skill
→ produces media intent + canvas context
→ provider router chooses configured provider / model
→ provider adapter calls actual API
→ canvas bridge writes result back to board
```

也就是说，**skill 负责理解任务，provider router 负责选择模型，canvas bridge 负责读写白板**。
用户调用哪个场景 skill，不等于底层必须使用哪个 provider。

插件安装完成后，需要自动弹出 onboarding，让用户先完成默认 provider 和模型配置。这个 onboarding 不应该藏在白板按钮里，而应该发生在 Codex 插件安装 / 首次启用流程中。

Onboarding 至少需要让用户选择：

| 配置项 | 推荐默认 | 可选项 |
|---|---|---|
| Image model provider | Atlas Cloud | Atlas Cloud / Codex GPT Image / Google Nano Banana / OpenAI-compatible / Custom |
| Video model provider | Atlas Cloud | Atlas Cloud / Google Veo / Seedance / Kling / OpenAI-compatible / Custom |
| Default image model | GPT Image 2 | provider 暴露的可用 image model |
| Default video model | Seedance 2.0 | provider 暴露的可用 video model |
| Credential / endpoint | 按 provider 要求填写 | API key、base URL、MCP server、CLI command 等 |

运行时 provider 选择优先级：

1. 用户本轮明确指定的 provider / model，例如“这次用 Kling 3”；
2. active skill 的安全建议或强需求，例如某个 skill 只支持视频；
3. onboarding 中配置的默认 provider / model；
4. 安全 fallback：能用 Codex native 就用 Codex native，否则向用户说明缺少 provider 配置。

用户必须能在 Codex 中随时查看和修改默认配置，例如：

- “查看当前默认图像 / 视频模型”；
- “把视频默认 provider 改成 Kling”；
- “这次用 Veo，但不要改默认设置”；
- “重设 Atlas key”；
- “添加一个公司内部 provider”。

自定义 provider 不是要求用户改源码，而是填写一份 provider config：
...
interface CustomProviderConfig {
  ...
  models: Array<{
    id: string
    displayName: string
    inputModes: Array<'text' | 'image' | 'video' | 'audio' | 'model3d'>
    outputMediaType: 'image' | 'video' | 'audio' | 'model3d'
  }>
  resultContract: 'local-file' | 'remote-url-to-materialize'
}
```

安全要求：

- API key 只能存在本地安全配置或用户明确授权的位置；
- key 不写入 canvas snapshot、frame input、generation request JSON 或 git repo；
- frame input 只引用本地素材路径 / asset id / metadata，不包含 provider secret；
- writeback 只接收已经 materialize 到本地的结果，或可被安全下载到本地的 remote URL。

画布 UI 只需要轻量展示当前 active provider / model 或 generation metadata，不应该把画布变成 provider playground。provider 配置主入口仍然在 Codex 插件 onboarding / settings 中。
...
公开仓库中保留实现规格和交接文档；早期 prototype/reference repos 仅作为本地历史资料，不随公开主仓库发布：

- `docs/codex-media-canvas-spec.md`：更接近实现约束的“防偏离版”规格；
- 早期失败原型、workflow graph 实验和竞品参考材料只用于本地回溯，不作为 CoFlow runtime 或公开包内容。
...
→ skill 产出 media intent，并交给 provider router 选择默认或指定 provider / model
→ provider adapter 调用实际 API / MCP / CLI 生成结果
→ skill 调 canvas.create_version / canvas.place_media 回板
```

这个项目需要定义的是 **canvas MCP contract + media skill contract + provider router contract**，不是定义所有 Skill。

Skill 只要遵守以下契约，就可以驱动画布：

- 能读取 selection 或 frame context；
- 能理解 media asset / annotation / version metadata；
- 能产出 media generation / edit intent，或直接产出一个或多个本地媒体文件；
- 如果需要调用模型，优先使用用户 onboarding 配置的默认 provider / model，而不是把 provider 写死在 skill 内；
...
interface CanvasAgentSkillManifest {
  ...
  providerPolicy?: {
    mediaTypes: Array<'image' | 'video' | 'audio' | 'model3d'>
    defaultProvider?: 'user-default' | 'skill-recommended' | string
...
这意味着未来用户即使调用的是 Higgsfield、营销素材、产品摄影、短视频脚本等第三方或场景 skill，只要它遵守 media skill contract，就可以继续复用当前 canvas bridge 和用户默认 provider 配置。场景 skill 决定“要做什么”，provider router 决定“用谁来生成”。
...
| provider onboarding 与默认模型配置 | Phase 2.1 | `provider-settings.json` / `canvas.get_provider_settings` / `canvas.get_provider_onboarding` / `canvas.set_provider_settings` 已作为本地 settings contract；用户能设置 image/video 默认 provider 与默认模型；`skip for now` / `rerun setup` 通过同一个 provider setup skill 实现；后续任意场景 skill 都优先走用户默认配置，并支持本轮覆盖。如果 Codex 插件生命周期暂时没有 first-enable hook，则 open skill 读取 onboarding payload 并在 `not_started` 时提示 |
...
| Atlas provider 链路 | Phase 1/2.1 | Phase 1 已跑通首个 image/video 真实生成与 writeback；Phase 2.1 继续校准 provider settings、API 字段、timing 与失败诊断 |
...
- 视频：默认模型 `Seedance 2.0`；有图片/视频/音频/3D 等参考时走 `reference_to_video`；无参考时走 `text_to_video`；默认比例优先读取用户提示词中的明确比例，其次使用 provider 支持的自适应 / 跟随参考素材能力；默认时长 `5s`；默认开启 audio；分辨率、码率、水印、末帧返回等参数必须以当前 Atlas Cloud 模型 API 文档为准，不能在代码或文档里自行假设不存在的字段。
...
### Phase 3：Provider router / Atlas / onboarding 与血缘增强，约 1 周

目标：

让 Atlas 成为最佳支持 provider，同时保持 open provider protocol，并把用户安装后的默认 provider / model 配置闭环。

范围：

- 插件安装后自动弹出 provider onboarding；
- 用户可配置默认 image provider / model 与 video provider / model；
- 用户可在 Codex 中查看、修改、重设 provider / model / key；
- provider router 按“本轮明确指定 → skill policy → 用户默认配置 → 安全 fallback”的顺序选择模型；
- Atlas image generate / edit；
- Atlas image-to-video；
- media upload；
- provider capability discovery；
- provider fallback；
- Atlas image / video polling 默认间隔统一为 `2000` ms，并保留后续 webhook / SSE 作为云部署或高级配置路径；
- 失败重试；
- 换模型重跑但不覆盖旧版本；
- cost / latency display；
- lineage panel 或可选 graph view；
- custom provider config 与 result contract 文档。
...
- 任意遵守 media skill contract 的场景 skill 都能使用用户默认 provider / model；
- 用户能在不改源码的情况下增加或切换 custom provider；
...
- 早期失败原型、竞品参考和 workflow graph 实验：仅作为本地历史资料，不放入公开主仓库。
...
11. 任何对白板元素、媒体素材、asset、shape、binding、frame、selection、copy/export/import/writeback 的修改，必须先查 tldraw 官方源码、官方 schema 或官方 starter 的实现方式；默认沿用 tldraw 原生 asset/shape/binding/store 构造，只在官方结构无法表达产品需求时才增加自定义字段或自定义 shape，并且必须写明原因与兼容边界。
12. MVP 不追求全能，而追求一个闭环打磨到顺：frame/selection、标注、生成、回板、血缘。
...
- CoFlow plan: [`docs/codex-media-canvas-plan.md`](./codex-media-canvas-plan.md)
