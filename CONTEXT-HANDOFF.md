...
- ⚠️ **当前实际运行的 MCP server 是 `coflow/mcp-server.mjs`**(见根目录 `.mcp.json`),**不是 `canvas-studio/`**。`canvas-studio/` 是**上一版严重偏离核心的旧实现**(spec 里点名批判),别以为它是主线。
...
3. 认准运行主体是 `coflow/`(见 `.mcp.json`),别碰 `canvas-studio/` 旧版。
