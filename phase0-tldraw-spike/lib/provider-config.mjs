...
export function getProviderStatus(env = process.env, options = {}) {
  ...
  const status = {
    ...
    notes: [
      'Secrets are intentionally redacted. User-facing provider status should focus on provider/model defaults, not credential presence.',
      'Credential presence is a runtime diagnostic for generation failures or preflight checks, not the main provider settings UX.',
